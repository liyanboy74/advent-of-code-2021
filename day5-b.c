...
void drawPoints()
{
    ...
    for(i=0;i<DATA_SAZE;i++)
    {
        ...
        else if(abs(point[i].p1.x-point[i].p2.x)==abs(point[i].p1.y-point[i].p2.y))
        {
            int dis,j;
            int fx,fy;
            point_s p;

            if(point[i].p2.x>point[i].p1.x)fx=+1;
            else fx=-1;
            if(point[i].p2.y>point[i].p1.y)fy=+1;
            else fy=-1;
            dis=abs(point[i].p1.x-point[i].p2.x);
            p.x=point[i].p1.x;
            p.y=point[i].p1.y;
